...
class ThemeSelector(tkinter.ttk.Frame):

    def __init__(self):
        tkinter.ttk.Frame.__init__(self, borderwidth=3)
    ...
    def _setup_widgets(self):
        themes_lbl = tkinter.ttk.Label(self, text="Themes")

        themes = self.style.theme_names()
        self.themes_combo = tkinter.ttk.Combobox(
            self, values=themes, state="readonly")
        self.themes_combo.set(themes[0])
        self.themes_combo.bind("<<ComboboxSelected>>", self._theme_sel_changed)

        change_btn = tkinter.ttk.Button(self, text='Change Theme',
                                        command=self._change_theme)

        theme_change_checkbtn = tkinter.ttk.Checkbutton(self,
